    ...
    <a-tabs default-active-key="1" @change="callback" class="padding-left">
      <a-tab-pane key="1" tab="快手">
        <div class="product-wrap">
                  ...
                  <a-col :span="6">
                    <a-form-model-item label="商品" style="width: 100%">
                      <a-select
                        ...
                        @change="(name) => handleUserChangeList(name, 'shopName')"
                  ...
                  <a-col :span="6">
                    <a-form-model-item label="推广位" style="width: 100%">
                      <a-select
                        ...
                        option-label-prop="label"
                        @search="(name) => formFuzzySearchList(name)"
                        @change="(name) => handleUserChangeList(name, 'promotionOptions')"
                        placeholder="请输入推广位ID/名称"
                      >
                        <a-spin v-if="listFetching" slot="notFoundContent" size="small" />
                        <a-select-option
                          v-for="(item, index) in promotionOptions"
                          :value="item.employee"
                          :key="item.id"
                          :label="item.realName"
                        >
                  ...
                  <a-col :span="6">
                    <a-form-model-item label="投放人员" style="width: 100%;">
                      <a-select
                        ...
                        option-label-prop="label"
                        @search="(name) => formFuzzySearchList(name)"
                        @change="(name) => handleUserChangeList(name, 'deliveryPersonnel')"
                        placeholder="工号/姓名"
                      >
                        <a-spin v-if="listFetching" slot="notFoundContent" size="small" />
                        <a-select-option
                          v-for="(item, index) in DeliveryPersonnelOptions"
                          :value="item.employee"
                          :key="item.id"
                          :label="item.realName"
                        >
                  ...
                  <a-col :span="6">
                    <a-form-model-item label="计划" style="width: 100%">
                      <a-select
                        ...
                        option-label-prop="label"
                        @search="(name) => formFuzzySearchList(name)"
                        @change="(name) => handleUserChangeList(name, 'plan')"
                  ...
                  <a-col :span="6">
                    <a-form-model-item label="账户ID" style="width: 100%">
                      <a-select
                        ...
                        option-label-prop="label"
                        @search="(name) => formFuzzySearchList(name)"
                        @change="(name) => handleUserChangeList(name,'accountID')"
                        placeholder="账户ID"
                      >
                        <a-spin v-if="listFetching" slot="notFoundContent" size="small" />
                        <a-select-option
                          v-for="(item, index) in accountIdOptons"
                          :value="item.employee"
                          :key="item.id"
                          :label="item.realName"
                        >
          ...
          <div class="content">
            <kwaikList :filterForm="filterForm" ref="kwaikList" />
          </div>
        </div>
      </a-tab-pane>
      <a-tab-pane key="2" tab="优雅">
        <div class="product-wrap">
          <div class="filter">
            <div class="left">
              <!-- 搜索表单内容 -->
          ...
          <div class="content">
            <kwaikList :filterForm="filterForm" ref="kwaikList" :isVisible="false" />
          </div>
        </div>
      </a-tab-pane>
    </a-tabs>
...
<script>
  ...
  import kwaikList from './components/kwai-list'
  export default {
    components: {
      kwaikList,
    },
    ...
    methods: {
      ...
      // 用户模糊搜索发生改变
      handleUserChangeList(item, key) {
        if (item === '' || item === undefined) return false
        this.filterForm[key] = item.split('/')[0]
      },
      // 时间选择器
      changeDatePick(date) {
        if (date.length) {
